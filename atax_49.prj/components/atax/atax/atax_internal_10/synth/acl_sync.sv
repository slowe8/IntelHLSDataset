...
module acl_sync #(
    //fifo config
    parameter int NUM_IFACES,               // >= 2, how many upstream interfaces, for only 1 interface use acl_desync
    ...
    parameter int EMPTY_LATENCY,            // latency from i_empty/i_almost_empty ports (from upstream fifos) to central_empty/central_almost_empty signals (input to slow read/fast read state machine)
    parameter int STALL_LATENCY,            // latency from central_forced_read signal (from slow read/fast read state machine) to o_stall port (stall going to upstream fifos)
    parameter int DATA_LATENCY,             // latency from i_data port (from upstream fifos) to o_data port (to downstream fifo), same as latency from upstream read to downstream write
    parameter int FULL_LATENCY,             // latency from i_stall port (almost full from downstream fifo) to central_almost_full signal (input to slow read/fast read state machine)
...
module acl_sync_predicate_nonblocking #(
    //fifo config
    parameter int NUM_IFACES,               // how many upstream interfaces: 2 or larger, 1 will produce functionally correct code but in practice we would always use desync
    parameter int DATA_WIDTH,               // data width of each upstream block, if an upstream blocks has a narrower width then quartus will prune away the unused bits
    ...
    //stall latency fast read/slow read state machine config
    parameter int EMPTY_PLUS_STALL_LATENCY, // the round trip latency from fast read/slow read state machine through upstream i_stall through upstream o_empty and back to fast read/slow read state machine
...
(
    ...
    generate
        ...
        for (f=0; f<NUM_IFACES; f=f+1) begin : GEN_RANDOM_BLOCK_NAME_R65
...
module acl_sync_stall_latency #(
    ...
    parameter int EMPTY_PLUS_STALL_LATENCY, // round-trip latency from the almost_empty of upstream fifo to centralized fast read/slow read state machine to i_stall of upstream fifo
