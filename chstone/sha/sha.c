...
static void sha_transform () {
  ...
  trans_lp1: for (i = 0; i < 16; ++i)
    {
      W[i] = sha_info_data[i];
    }
  trans_lp2: for (i = 16; i < 80; ++i)
    {
  ...
  trans_lp3: for (i = 0; i < 20; ++i)
    {
      FUNC (1, i);
    }
  trans_lp4: for (i = 20; i < 40; ++i)
    {
      FUNC (2, i);
    }
  trans_lp5: for (i = 40; i < 60; ++i)
    {
      FUNC (3, i);
    }
  trans_lp6: for (i = 60; i < 80; ++i)
    {
      FUNC (4, i);
    }
...
component void sha_stream ()
{
  int i, j;
  const BYTE *p;

  sha_init ();
  main_loop:for (j = 0; j < VSIZE; j++)
    {
      i = in_i[j];
      p = &indata[j][0];
      sha_update (p, i);
