...
for ((i = $bench_num1 ; i < (($bench_num2 + 1)) ; i++)); do
	...
	if [ -f "$FILE" ]; then
		...
		quartus_sh -t $base_dir/power_to_csv.tcl -project "quartus_compile" -panel "Power Analyzer||Power Analyzer Summary" -file ../../../data/"$benchmark"_"$i".csv
  		
		cd $base_dir/$benchmark_family
		python3 create_tables.py $benchmark $benchmark/intelversions/"$benchmark"_"$i".prj
