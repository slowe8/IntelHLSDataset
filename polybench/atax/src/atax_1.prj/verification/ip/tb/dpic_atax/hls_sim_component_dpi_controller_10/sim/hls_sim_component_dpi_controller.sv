...
#(
    ...
    // if component is slave component, use slave_busy signal instead
    // slave_busy inditicates that component is under reset or in cycles required for the node to exit safe state following the deassertion reset 
    assign do_busy = (COMPONENT_HAS_SLAVE_RETURN == 1) ? slave_busy : busy;
    ...
    (
        .source_data    (),
        .source_ready   (~do_busy & slaves_ready_stable),
    ...
    logic is_done;
    assign is_done = (COMPONENT_HAS_SLAVE_RETURN == 1) ? done_irq_edge : (done & ~stall);
    ...
    always @(posedge clock or negedge resetn) begin
      if (!resetn) begin
        ...
        if (!component_enabled)
          ...
          readback_from_slaves <= (component_latency_tracker_inst.num_threads == 0) & ~do_start;
      end
    end
