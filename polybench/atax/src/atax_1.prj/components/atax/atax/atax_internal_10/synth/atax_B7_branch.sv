...
module atax_B7_branch (
    input wire [31:0] in_c0_exe188,
    input wire [0:0] in_cmp25,
    input wire [31:0] in_inc38,
    ...
    reg [31:0] c0_exe188_reg_q;
    wire [0:0] cmp25_cmp_q;
    reg [31:0] inc38_reg_q;
    ...
    // cmp25_cmp(LOGICAL,5)
    assign cmp25_cmp_q = ~ (in_cmp25);

    // valid_out_1_and(LOGICAL,27)
    assign valid_out_1_and_q = in_valid_in & cmp25_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_cmp25;
    ...
    begin
        if (!resetn)
        begin
            c0_exe188_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (atax_B7_branch_enable_q == 1'b1)
        begin
            c0_exe188_reg_q <= $unsigned(in_c0_exe188);
        end
    end

    // out_c0_exe188(GPOUT,19)
    assign out_c0_exe188 = c0_exe188_reg_q;
    ...
    begin
        if (!resetn)
        begin
            inc38_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (atax_B7_branch_enable_q == 1'b1)
        begin
            inc38_reg_q <= in_inc38;
        end
    end

    // out_inc38(GPOUT,20)
    assign out_inc38 = inc38_reg_q;
