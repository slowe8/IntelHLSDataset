...
module atax_B9_branch (
    input wire [0:0] in_c0_exe2194,
    input wire [0:0] in_c0_exe3195,
    input wire [0:0] in_c0_exe4196,
    input wire [0:0] in_c0_exe5197,
    ...
    reg [0:0] c0_exe2194_reg_q;
    wire [0:0] c0_exe3195_cmp_q;
    reg [0:0] c0_exe4196_reg_q;
    reg [0:0] c0_exe5197_reg_q;
    ...
    // c0_exe3195_cmp(LOGICAL,5)
    assign c0_exe3195_cmp_q = ~ (in_c0_exe3195);

    // valid_out_1_and(LOGICAL,30)
    assign valid_out_1_and_q = in_valid_in & c0_exe3195_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe3195;
    ...
    // c0_exe2194_reg(REG,4)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe2194_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe2194_reg_q <= in_c0_exe2194;
        end
    end

    // out_c0_exe2194(GPOUT,21)
    assign out_c0_exe2194 = c0_exe2194_reg_q;

    // c0_exe4196_reg(REG,6)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe4196_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe4196_reg_q <= in_c0_exe4196;
        end
    end

    // out_c0_exe4196(GPOUT,22)
    assign out_c0_exe4196 = c0_exe4196_reg_q;

    // c0_exe5197_reg(REG,7)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe5197_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe5197_reg_q <= in_c0_exe5197;
        end
    end

    // out_c0_exe5197(GPOUT,23)
    assign out_c0_exe5197 = c0_exe5197_reg_q;
