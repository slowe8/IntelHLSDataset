...
// Profiling
output wire extra_unaligned_reqs;
...
begin
   if (~aclrn) reg_lsu_i_valid <= 1'b0; 
   else begin
      if (~o_stall)   reg_lsu_i_valid <= i_valid;
      if (~sclrn[0])  reg_lsu_i_valid <= '0;
   end
end
...
(
    clk, clk2x, resetn, o_stall, i_valid, i_address, i_writedata, i_stall, i_byteenable, o_valid, 
    o_active, //Debugging signal
    avm_address, avm_write, avm_writeack, avm_writedata, avm_byteenable, avm_waitrequest,
    avm_burstcount,
    i_nop,
