...
module atax_B9_branch (
    input wire [0:0] in_c0_exe2215,
    input wire [0:0] in_c0_exe3216,
    input wire [0:0] in_c0_exe4217,
    input wire [0:0] in_c0_exe5218,
    ...
    reg [0:0] c0_exe2215_reg_q;
    wire [0:0] c0_exe3216_cmp_q;
    reg [0:0] c0_exe4217_reg_q;
    reg [0:0] c0_exe5218_reg_q;
    ...
    // c0_exe3216_cmp(LOGICAL,5)
    assign c0_exe3216_cmp_q = ~ (in_c0_exe3216);

    // valid_out_1_and(LOGICAL,30)
    assign valid_out_1_and_q = in_valid_in & c0_exe3216_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe3216;
    ...
    // c0_exe2215_reg(REG,4)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe2215_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe2215_reg_q <= in_c0_exe2215;
        end
    end

    // out_c0_exe2215(GPOUT,21)
    assign out_c0_exe2215 = c0_exe2215_reg_q;

    // c0_exe4217_reg(REG,6)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe4217_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe4217_reg_q <= in_c0_exe4217;
        end
    end

    // out_c0_exe4217(GPOUT,22)
    assign out_c0_exe4217 = c0_exe4217_reg_q;

    // c0_exe5218_reg(REG,7)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe5218_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe5218_reg_q <= in_c0_exe5218;
        end
    end

    // out_c0_exe5218(GPOUT,23)
    assign out_c0_exe5218 = c0_exe5218_reg_q;
