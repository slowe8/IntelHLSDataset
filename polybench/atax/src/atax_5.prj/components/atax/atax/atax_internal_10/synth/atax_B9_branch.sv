...
module atax_B9_branch (
    input wire [0:0] in_c0_exe2206,
    input wire [0:0] in_c0_exe3207,
    input wire [0:0] in_c0_exe4208,
    input wire [0:0] in_c0_exe5209,
    ...
    reg [0:0] c0_exe2206_reg_q;
    wire [0:0] c0_exe3207_cmp_q;
    reg [0:0] c0_exe4208_reg_q;
    reg [0:0] c0_exe5209_reg_q;
    ...
    // c0_exe3207_cmp(LOGICAL,5)
    assign c0_exe3207_cmp_q = ~ (in_c0_exe3207);

    // valid_out_1_and(LOGICAL,30)
    assign valid_out_1_and_q = in_valid_in & c0_exe3207_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe3207;
    ...
    // c0_exe2206_reg(REG,4)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe2206_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe2206_reg_q <= in_c0_exe2206;
        end
    end

    // out_c0_exe2206(GPOUT,21)
    assign out_c0_exe2206 = c0_exe2206_reg_q;

    // c0_exe4208_reg(REG,6)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe4208_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe4208_reg_q <= in_c0_exe4208;
        end
    end

    // out_c0_exe4208(GPOUT,22)
    assign out_c0_exe4208 = c0_exe4208_reg_q;

    // c0_exe5209_reg(REG,7)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe5209_reg_q <= $unsigned(1'b0);
        end
        else if (atax_B9_branch_enable_q == 1'b1)
        begin
            c0_exe5209_reg_q <= in_c0_exe5209;
        end
    end

    // out_c0_exe5209(GPOUT,23)
    assign out_c0_exe5209 = c0_exe5209_reg_q;
